...
df %>%
  ...
  annotate("text",
           x = mean(df$dol_nicheolap, na.rm = TRUE) + 0.08,
           y = 200,
           size = 4,
           label = paste("Mean =", round(mean(df$dol_nicheolap, na.rm = TRUE), 2))) +
  geomtextpath::geom_textvline(xintercept = 0,
                               label = "RSS (2012)",
                               linetype = 3,
                               hjust = 0.8,
                               size = 4) +
  geomtextpath::geom_textvline(xintercept = 0.49,
                               label = "NATO (2012)",
                               linetype = 3,
                               hjust = 0.8,
                               size = 4) +
  geomtextpath::geom_textvline(xintercept = 0.88,
                               label = "Cuba-North Korea (2012)",
                               linetype = 3,
                               hjust = 0.8,
                               size = 4) +
...
ggsave("fig-dv-1.png", width = 8, height = 6)
```
...
df |>
  dplyr::select(year,
                ...
                hier_laplace) |>
  ggplot(aes(x = year, 
             y = strcomp_cinc_total)) + 
  geom_line(aes(group = atopid),
            size = 0.8) +
  gghighlight::gghighlight(atopid == 3995,
                           use_direct_label = T, 
  ...
  scale_x_continuous(expand = c(0, 0),
                     breaks = seq(1970, 2014, 5),
                     limits = c(1970, 2020)) +
  labs(title = "Alliance Strategic Compatibility (1970-2014)", x = "Year", y = "Strategic Compatibility") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5),
        text = element_text(size = 14))

ggsave("fig-ev-1.png", width = 8, height = 6)
```
...
df <- dplyr::left_join(country_year, laplace)

names <- sipri |>
  dplyr::select(year, sender, ccode1) |>
  dplyr::distinct() |>
  dplyr::rename(ccode = ccode1) |>
  dplyr::mutate(ccode = as.character(ccode))

df <- dplyr::left_join(df, names) |>
  dplyr::mutate(sender = ifelse(ccode == 640, "Turkey", sender),
                sender = ifelse(ccode == 349, "Slovenia", sender))

df |>
  dplyr::arrange(laplace) |>
  dplyr::mutate(sender = factor(sender, levels = sender)) |>
  ggplot(aes(x = laplace, 
             y = sender)) +
  geom_segment(aes(xend = 0, yend = sender)) +
  geom_point(size = 4, color = "orange") +
  labs(title = "NATO Laplacian entropy (2003) = 0.76",
       x = "Laplacian entropy",
       y = "Country") +
  theme_classic() +
  theme(text = element_text(size = 14))
ggsave("fig-ev-2.png", width = 8, height = 6)

mean(test$hier_laplace, na.rm = TRUE) + sd(test$hier_laplace, na.rm = TRUE)
```
...
m4_cse <- lmtest::coeftest(m1_yfe,
                              vcov = sandwich::vcovHC, type = "HC1",
                              save = TRUE)

models <- list(m1_bicse, m2_bicse, m3_bicse, m4_cse)

coefs <- c("strcomp_cinc_total" = "Strategic Compatibility",
           "hier_laplace" = "Hierarchy",
           "cinc_totalsq" = "CINC sq.",
...
ggsave("dol_coefplot.png", width = 7, height = 4)
```
