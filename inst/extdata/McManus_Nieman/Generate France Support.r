...
d$fra_pact <- ordered(as.factor(d$fra_pact))
d$fra_vis <- ordered(as.factor(d$fra_vis))
d$fra_ex <- ordered(as.factor(d$fra_ex))
...
# Estimate model
post_fra<- MCMCmixfactanal(~
fra_pact+log_fra_arms+log_fra_troops+fra_vis+fra_ex
...
factors=1, data=d,
lambda.constraints = list(fra_pact=list(2,"+")),
...
write.csv(lambda,file="lambda_fra.csv")
write.csv(phi,file="country_irt_fra.csv")
