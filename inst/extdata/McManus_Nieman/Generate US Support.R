...
d$us_pact <- ordered(as.factor(d$us_pact))
d$us_vis <- ordered(as.factor(d$us_vis))
d$us_ex <- ordered(as.factor(d$us_ex))
d$us_nukes <- ordered(as.factor(d$us_nukes))
...
us_pact+log_us_arms+log_us_troops+us_vis+us_ex+log_us_words+us_nukes
...
factors=1, data=d,
lambda.constraints = list(us_pact=list(2,"+")),
