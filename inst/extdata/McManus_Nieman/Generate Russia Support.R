...
## Russia signal
d <- read.csv("Individual Signals Dataset.csv", header=TRUE)
row.names(d) <- d$ctry_yr

d$rus_pact <- ordered(as.factor(d$rus_pact))
d$rus_vis <- ordered(as.factor(d$rus_vis))
d$rus_ex <- ordered(as.factor(d$rus_ex))
d$rus_nukes <- ordered(as.factor(d$rus_nukes))
...
# Estimate model
post_rus <- MCMCmixfactanal(~
rus_pact+log_rus_arms+log_rus_troops+rus_vis+rus_ex+rus_nukes
...
factors=1, data=d,
lambda.constraints = list(rus_pact=list(2,"+")),
...
write.csv(lambda,file="lambda_rus.csv")
write.csv(phi,file="country_irt_rus.csv")
